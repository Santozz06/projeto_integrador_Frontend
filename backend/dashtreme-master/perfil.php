...
try {
    if ($tipoUsuario === 'professor') {
        ...
        $sqlTurma = "SELECT t.Nome_Turma, t.Turno, t.Ano_Letivo, t.Etapa
                     FROM Matriculas m
...
<body class="bg-theme bg-theme1 <?= htmlspecialchars($bodyClass) ?>">
    <?php
      ...
      $menuFile = __DIR__ . '/' . $menuPath;
      if (file_exists($menuFile)) {
          require $menuFile;
      } else {
          echo '<!-- Menu não encontrado: ' . htmlspecialchars($menuFile) . ' -->';
      }
    ?>
            ...
            <div class="dados-topo">
                ...
                <div class="dados-profissionais">
                    ...
                    <?php else: ?>
                        ...
                        <p><strong>Nome do Responsável:</strong> <?= htmlspecialchars($usuario['Nome_Responsavel'] ?? 'Não informado') ?></p>
                        <p><strong>Telefone Responsável:</strong> <?= htmlspecialchars($usuario['Telefone_Responsavel'] ?? 'Não informado') ?></p>
                        <p><strong>Data de Ingresso:</strong> <?= htmlspecialchars($usuario['Data_Ingresso'] ?? 'Não informado') ?></p>
                    <?php endif; ?>
                </div>
            </div>
