...
try {
    // frequencia da turma usando a tabela Presencas
    $params = [$idTurma];
    $dateWhere = '';
    if ($dataInicio) { $dateWhere .= ' AND p.Data >= ?'; $params[] = $dataInicio; }
    if ($dataFim) { $dateWhere .= ' AND p.Data <= ?'; $params[] = $dataFim; }
    ...
    $sql = "SELECT 
                ...
                SUM(CASE WHEN p.Status = 'A' THEN 1 ELSE 0 END) AS Faltas,
                SUM(CASE WHEN p.Status = 'J' THEN 1 ELSE 0 END) AS Justificadas,
            ...
            LEFT JOIN Presencas p ON p.ID_Matricula = m.ID_Matricula $dateWhere
