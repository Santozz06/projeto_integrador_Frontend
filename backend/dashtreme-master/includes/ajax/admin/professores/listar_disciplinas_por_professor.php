...
try {
        // tentamos primeiro via Horarios (melhor fidelidade por ano)
        $paramsH = [];
        $sqlH = "SELECT h.ID_Professor,
              ...
              WHERE 1=1";
        if ($ano) { $sqlH .= " AND h.Ano_Letivo = ?"; $paramsH[] = $ano; }
        if ($disciplina) { $sqlH .= " AND d.Nome_Disciplina = ?"; $paramsH[] = $disciplina; }
        if ($professorId) { $sqlH .= " AND h.ID_Professor = ?"; $paramsH[] = $professorId; }
        $sqlH .= " GROUP BY h.ID_Professor";

        // se Horarios não tiver dados, cai para Disciplinas
        $paramsD = [];
    ...
    // decide fonte principal: Horarios > Disciplinas > Fallback
    $useHorarios = false;
    $useFallback = false;
    try {
        $cntH = "SELECT COUNT(*) FROM Horarios h INNER JOIN Disciplinas d ON d.ID_Disciplina=h.ID_Disciplina WHERE 1=1";
        $pH = [];
        if ($ano) { $cntH .= " AND h.Ano_Letivo = ?"; $pH[] = $ano; }
        if ($disciplina) { $cntH .= " AND d.Nome_Disciplina = ?"; $pH[] = $disciplina; }
        if ($professorId) { $cntH .= " AND h.ID_Professor = ?"; $pH[] = $professorId; }
        $cH = $pdo->prepare($cntH); $cH->execute($pH);
    ...
    $sqlDF .= " GROUP BY pt.ID_Professor";
        ...
        $sqlTH = "SELECT h.ID_Professor,
            ...
            WHERE 1=1";
        if ($ano) { $sqlTH .= " AND h.Ano_Letivo = ?"; $paramsTH[] = $ano; }
        if ($professorId) { $sqlTH .= " AND h.ID_Professor = ?"; $paramsTH[] = $professorId; }
        ...
        if ($professorId) { $sqlT .= " AND pt.ID_Professor = ?"; $paramsT[] = $professorId; }
